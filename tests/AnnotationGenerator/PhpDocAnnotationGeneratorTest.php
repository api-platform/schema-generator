...
class PhpDocAnnotationGeneratorTest extends TestCase
{
    ...
    {
        yield 'with interface' => [(new Class_('Res', new Resource('https://schema.org/Res')))->withInterface(new Interface_('Interface', '/foo')), ['{@inheritdoc}', '', '@author Bill']];

        $graph = new Graph();
        yield 'with resource' => [new Class_('Res', new Resource('https://schema.org/Res', $graph)), ['', '', '@see https://schema.org/Res', '@author Bill']];
    }
}
